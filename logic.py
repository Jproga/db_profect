from new.config import TOKEN, DATABASE
import sqlite3
...
class DB_Manager:
    def __init__(self, database):
        self.database = database #имя базы
    ...
    def create_tables(self):
        ...
        with connection:
            ...
            sql =  """CREATE TABLE status (
                ...
                status_name TEXT)"""
            connection.execute(sql)
            sql = """CREATE TABLE project_skills (
                    project_id INTEGER,
                    skill_id INTEGER,
    ...
    def insert_project(self, data):
        sql = "INSERT INTO projects (user_id, project_name, description, url) VALUES (?,?,?,?)"
        self.__executemany(sql, data)
    ...
    def get_projects(self, user_id):
        sql = "SELECT * FROM projects WHERE project_name" # Запиши сюда правильный SQL запрос
...
if __name__ == "__main__":
    ...
    #db.default_insert()
    #(user_id, project_name, description, url)
    #db.insert_project([(1,"Мой сайт","описание","https:/")])
    #print(db.get_statuses())
    #print(db.get_skills())
    #db.create_tables()
    #db.delete_project(2,2)
    #db.delete_skill(1,1)
